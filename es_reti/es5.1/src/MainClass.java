...
public class MainClass {
    ...
    public static void main(String[] args) throws IOException {
        String basedir = "/Users/Marta/Desktop/INFOrno/fatti";

        File startDirectory = new File(basedir);

        //if startDirectory does not exist
        if(!startDirectory.exists()) {
            System.out.println("Il file iniziale non esiste");
            System.exit(-1);
        }

        //if startDirectory is not a directory
        if(!startDirectory.isDirectory()) {
            System.out.println("Il file iniziale non è una directory");
            System.exit(-1);
        }

        //definisco lista directories in cui salvo un pezzo di percorso in ogni struct
